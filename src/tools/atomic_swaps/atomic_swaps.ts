...
import { 
  ...
  balanceSchema, 
  errorSchema 
} from "./schemas/swap.js";
import {
  satoshisToHumanReadable,
...
export class AtomicSwapsTool {
  ...
  constructor(config: AtomicSwapConfig = createDefaultConfig(), nwcClient: nwc.NWCClient) {
  ...
  registerTool(server: McpServer): void {
    server.registerTool(
      "starknet_lightning_atomic_swaps",
      {
        ...
        description: "Execute trustless atomic swaps between Lightning Network and Starknet using AtomiqLabs",
        inputSchema: {
          action: z.enum([
            "quote", 
            "execute", 
          ...
          // Common parameters
          direction: z.enum(["lightning_to_starknet", "starknet_to_lightning"])
            .describe("Direction of the swap")
            .optional(),
          
          // Quote and execute parameters
          amount: z.string()
            .describe("Amount to swap (in human readable format, e.g., '0.001' for BTC or '1.5' for STRK)")
            .optional(),
          exact_in: z.boolean()
            .describe("Whether amount is exact input (true) or exact output (false)")
            .default(true)
            .optional(),
          
          // Addresses
          lightning_invoice: z.string()
            .describe("Lightning invoice for payment (for starknet_to_lightning)")
            .optional(),
          lightning_address: z.string()
            .describe("Lightning address or LNURL for payment (for starknet_to_lightning)")  
            .optional(),
          starknet_address: z.string()
            .describe("Starknet address for receiving funds (for lightning_to_starknet)")
            .optional(),
          ...
          // Parse address
          address: z.string()
            .describe("Address to parse (Lightning invoice, LNURL, Bitcoin address, etc.)")
            .optional(),
        },
        outputSchema: {
          success: z.boolean(),
      ...
      async (params) => {
        try {
          console.log("AtomicSwapsTool received params:", JSON.stringify(params, null, 2));
          ...
          return {
            ...
            structuredContent: JSON.parse(JSON.stringify(result, (key, value) => 
          ...
          const errorResult = {
            ...
            error: {
              ...
              recovery_suggestion: this.getRecoverySuggestion(error instanceof Error ? error : new Error(String(error)))
            }
          };
          ...
          return {
            content: [{
              type: "text", 
              text: JSON.stringify(errorResult, (key, value) => 
                typeof value === 'bigint' ? value.toString() : value, 2)
            }],
            structuredContent: JSON.parse(JSON.stringify(errorResult, (key, value) => 
              typeof value === 'bigint' ? value.toString() : value))
          };
        }
      }
  ...
  private async handleQuote(params: any): Promise<any> {
    ...
    if (params.direction === "lightning_to_starknet") {
      ...
      if (!dstAddress) {
        throw new Error("Lightning invoice or address is required for starknet_to_lightning");
      }
    }
    ...
    return {
      ...
      data: {
        ...
        input_token: params.direction === "lightning_to_starknet" ? "BTC" : "STRK",
        output_token: params.direction === "lightning_to_starknet" ? "STRK" : "BTC",
  ...
  private async handleExecute(params: any): Promise<any> {
    ...
    const swap = this.activeSwaps.get(params.swap_id) || 
    ...
    if (params.direction === "lightning_to_starknet") {
      ...
      result = {
        success: true,
        message: "Lightning invoice created. Please pay the invoice to complete the swap.",
        data: {
   ...
   */
  private async handleLightningToStarknetSwap(swapId: string, params: any): Promise<any> {
    ...
    if (params.auto_pay !== false) {
      try {
        ...
        try {
          const { fees_paid, preimage, ...paymentResult } = await this.nwcClient.payInvoice({
            invoice: lightningInvoice
          });
          
          console.log("Lightning invoice payment result:", JSON.stringify({ fees_paid, preimage, ...paymentResult }, null, 2));
          ...
          processedResult = {
            ...
            payment_error: nwcError instanceof Error ? nwcError.message : String(nwcError)
          };
          console.log("Payment failed, will monitor swap for external payment:", processedResult);
        }
        ...
        const maxAttempts = 60; // 60 seconds timeout (increased for better chance)
        let attempts = 0;
        
        while (attempts < maxAttempts) {
          const state = swap.getState();
          console.log(`Swap state check (${attempts + 1}/${maxAttempts}):`, formatSwapState(state));
          
          // Check if the swap has progressed (state 1 = COMMITTED means payment received)
          if (state === 1) {
            console.log("Swap state changed to COMMITTED - payment received by AtomiqLabs");
          }
          
          if (state === 2 || state === 3) { // SOFT_CLAIMED or CLAIMED
            ...
            return {
              ...
              data: {
                ...
                next_steps: ["Swap completed successfully! STRK tokens should be in your wallet."]
              }
            };
          }
          ...
          if (state < 0) {
            console.error("Swap entered error state:", formatSwapState(state));
            return {
              success: false,
        ...
        // Check if payment succeeded but swap is taking longer
        const isPaymentSuccessful = processedResult.preimage && processedResult.preimage !== "";
        ...
        return {
          ...
          data: {
            ...
            next_steps: isPaymentSuccessful 
              ...
              : ["Pay the invoice manually if payment failed", "Check swap status periodically"]
          }
        };
        ...
        if (error instanceof Error && error.message.includes("no preimage received")) {
          return {
            success: false,
            message: "Lightning payment verification failed - payment may not have been made",
            data: {
              swap_id: swapId,
              state: formatSwapState(swap.getState()),
              ...
              error_details: {
                ...
                suggestion: "Payment may have failed - please check your Lightning wallet"
              },
              next_steps: [
        ...
        return {
          success: false,
          message: `Failed to pay Lightning invoice: ${error instanceof Error ? error.message : String(error)}`,
          data: {
            swap_id: swapId,
            state: formatSwapState(swap.getState()),
            lightning_invoice: lightningInvoice,
            next_steps: ["Try paying the invoice manually", "Check your Lightning wallet balance"]
          }
        };
      }
    }
    ...
    return {
      success: true,
      message: "Lightning to Starknet swap initiated. Please pay the invoice to complete the swap.",
      data: {
        swap_id: swapId,
        state: formatSwapState(swap.getState()),
        lightning_invoice: lightningInvoice,
        lightning_hyperlink: swap.getHyperlink?.() || `lightning:${lightningInvoice}`,
        next_steps: [
   ...
   */
  private async handleStarknetToLightningSwap(swapId: string, params: any): Promise<any> {
    const swap = this.activeSwaps.get(swapId);
    if (!swap) {
      throw new Error(`Swap with ID ${swapId} not found`);
    }
 ...
 */
export function registerAtomicSwapsTool(server: McpServer, nwcClient: nwc.NWCClient): void {
  try {
